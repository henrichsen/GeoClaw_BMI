...
def setrun(claw_pkg='geoclaw'):
    ...
    amrdata.refinement_ratios_x = [2, 5, 6, 4, 8]
    amrdata.refinement_ratios_y = [2, 5, 6, 4, 8]
    amrdata.refinement_ratios_t = [2, 5, 6, 4, 8]
    ...
    # to specify regions of refinement append lines of the form
    #  [minlevel,maxlevel,t1,t2,x1,x2,y1,y2]
    #rundata.regiondata.regions.append([44, 60, 0., 10000.,-95.2,-94.5,29.2, 30.5])
   # rundata.regiondata.regions.append([12, 15, 8000., 26000., -95.5,-94,-29,-31])
    
    rundata.regiondata.regions.append([6, 6, -259200, 756000,-95.5,-94,29, 30.1])
    ...
    rundata.gaugedata.gauges.append([4, -94.13, 29.58,
                                     ...
                                     rundata.clawdata.tfinal])
    #Gauge number 5
    rundata.gaugedata.gauges.append([5, -94.73, 29.36,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    
    # Observation points along the river
    rundata.gaugedata.gauges.append([6, -94.934, 29.674,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([7, -94.93, 29.74,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([8, -94.92, 29.80,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([9, -94.97, 29.86,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([10, -94.985, 29.97,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([11, -95.00, 30.11,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    
    rundata.gaugedata.gauges.append([12, -94.6922, 29.6516,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([13, -94.58783, 29.72694,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([14, -94.5295, 29.8751,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    
    rundata.gaugedata.gauges.append([15, -94.5065, 29.5994,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([16, -94.5344, 29.66259,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
    rundata.gaugedata.gauges.append([17, -94.5332, 29.9467,
                                     rundata.clawdata.t0,
                                     rundata.clawdata.tfinal])
...
def setgeo(rundata):
    ...
    refine_data.wave_tolerance = 0.50
    refine_data.speed_tolerance = [0.50, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0]
  ...
  #  clawutil.data.get_remote_file(
   #        "http://www.columbia.edu/~ktm2132/bathy/gulf_caribbean.tt3.tar.bz2")
    ...
    topo_path = os.path.join(scratch_dir, 'fine_galveston.tt3') # 3 Second
    topo_data.topofiles.append([3, topo_path])
    
    topo_path = os.path.join(scratch_dir, 'galveston_13_mhw_2007.tt3') # 1/3 Second
    topo_data.topofiles.append([3, topo_path])
    
   # topo_path = os.path.join(scratch_dir, 'USGS_Catchment_10m.tt3') # 1/3 Second
   # topo_data.topofiles.append([3, topo_path])
    
   # topo_path = os.path.join(scratch_dir, 'GalvestonBay.tt3')
   # topo_data.topofiles.append([3, topo_path])
    ...
    # Uncomment/comment out to use the old version of the harvey storm file
    # harvey = Storm(path="old_harvey.storm", file_format="ATCF")
    harvey = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info (8/26/2017 ~ 6 UTC)
    harvey.time_offset = datetime.datetime(2017, 8, 26, 6)
