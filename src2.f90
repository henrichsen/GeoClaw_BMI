   ...
   !     do k=1,tide_m
    ...
    if (.true.) then
       ...
       ! discharge_cfs = 0d0  ! cubic feet / second
        ...
        !print *,"mx: ",mx," my: ",my," xlower: ",xlower," ylower: ", ylower," dx: ",dx," dy: ", dy," x1: ",x1rs," x2: ",x2rs," y1: ",y1rs," y2: ", y2rs, " "
        do i=1,mx
            x = xlower + (i-0.5d0)*dx
            !print *,"x: ",x
            do j=1,my
                y = ylower + (j-0.5d0)*dy
                if ((x >= x1rs) .and. (x <= x2rs) .and. (y >= y1rs) .and. (y <= y2rs)) then
                    !print *," Q at x: ",x," y: ",y, "  is Defined"
